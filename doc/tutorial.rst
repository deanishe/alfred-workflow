...
=======================================================
Writing a Pinboard.in Workflow with **Alfred-Workflow**
=======================================================

In which we build an Alfred Workflow to view recent posts to
`Pinboard.in <https://pinboard.in/>`_.

**Note**: To use Workflows, you must own Alfred's
...
The next is to add a **Script Filter**:

.. image:: _static/screen3_add_script_filter.png

And enter the details for this action:
...
**Note:** You *can* choose ``/usr/bin/python`` as the **Language** and paste
...
all. You'll have to open the Workflow directory in Terminal and run the script
by hand to see the error::

	python pinboard.py

Making the Workflow useful
--------------------------
...
Add the highlighted lines to your ``pinboard.py`` file:
...
Now run your Workflow again in Alfred, select on of the results and hit
**ENTER**. The page should open in your default browser.
...
function (``get_recent_posts``) and instead we ask
:meth:`~workflow.workflow.Workflow.cached_data` for the data cached under
the name ``posts`` (the first argument). :meth:`~workflow.workflow.Workflow.cached_data`
will first check its cache for data saved under ``posts`` and return those data
...
immediately. Then add ``"{query}"`` in the **Script**. ``{query}`` will be
replaced by Alfred with whatever you've typed after the keyword. Finally, set
the **Escaping** options to:
...
and **nothing** else. This ensures that the query reaches your script
...
allowed) and to cache them for 10 minutes (or use 300 seconds for 5 minutes
if you're a heavy Pinboardista):
...
Looking at ``main`` first, we add a ``query`` variable. Because our Script Filter
can run with or without an argument, we test to see if any were passed to the
script using via :attr:`~workflow.workflow.Workflow.args` attribute of
:class:`~workflow.workflow.Workflow`, and grab the first one if there were
...
using the :meth:`~workflow.workflow.Workflow.filter` method of
:class:`~workflow.workflow.Workflow` if there is a ``query``.

:meth:`~workflow.workflow.Workflow.filter` implements an Alfred-like search
algorithm (e.g. "am" will match "Activity Monitor" as well as "I Am Legend"),
but it needs a string to search. Therefore, we write the ``search_key_for_post``
function that will build a searchable string for each post comprised of its
title, tags and description (in that order).

**Note:** In the last line of ``search_key_for_post``, we join the elements with
``u' '``, not ``' '``, which is a Unicode space, not an ASCII space. The
:meth:`~workflow.web.Response.json` method of :class:`~workflow.web.Response`
return Unicode (as do most **Alfred-Workflow** methods), and if you used an
ASCII space ``' '`` (or any ASCII string), your Workflow would throw an error
...
:meth:`~workflow.workflow.Workflow.decode` method to help you.
